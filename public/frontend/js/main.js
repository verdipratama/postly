  ...
  $(document).on("pjax:complete", function () {
    $("#video_upload").on("change", function (e) {
      ...
      $.ajax({
              ...
              if (evt.lengthComputable) {
                ...
                if (percentComplete.toFixed(0) == 100) {
                  ...
                  reader.onload = function (e) {
                    ...
                    $(".video-upload-input-area").html(
                      '<video controls autoplay muted="muted"><source src="' +
                        e.target.result +
        ...
        beforeSend: function () {
          $(".video-upload-input-area").html(
            '<div class="box"><div class="percent"><svg><circle cx="70" cy="70" r="70"></circle><circle cx="70" cy="70" r="70"></circle></svg></div><div class="number"><h2 class="video_percentence_number">0%</h2></div></div>'
          );
        },
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.errors) {
          ...
          $(".ellipish-modal-content").html(
            '<div class="upload_success"><i class="fas fa-check"></i><h4>Your video successfully uploaded</h4><a class="pjax" href="' +
              url +
              '/upload">Upload Another Video</a><a class="pjax" href="' +
              url +
              "/user/" +
              response +
              '">View Profile</a></div>'
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response == "night") {
        ...
        $("#notification_mode").attr(
          "src",
          baseurl + "/frontend/img/white_notification.png"
        );
        $("#upload_mode").attr(
          "src",
          baseurl + "/frontend/img/white_upload.png"
        );
        $("#mode_action").html(
          'Day Mode <div class="mode day"><i class="far fa-sun"></i></div>'
        );
      }
      ...
      if (response == "day") {
        ...
        $("#notification_mode").attr(
          "src",
          baseurl + "/frontend/img/notification.png"
        );
        $("#upload_mode").attr("src", baseurl + "/frontend/img/upload.png");
        $("#mode_action").html(
          'Night Mode <div class="mode night"><i class="far fa-moon"></i></div>'
        );
      }
    },
  });
